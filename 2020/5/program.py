...
seats = advent_lib.read_input()

def getPlace(seq: str) -> (int, int):
    row = ''.join(['1' if c == "B" else '0' for c in seq[:7]])
    col = ''.join(['1' if c == "R" else '0' for c in seq[7:]])
    return int(row, 2), int(col, 2)


def getSeatId(seq: str) -> int:
    row, col = getPlace(seq)
    return row * 8 + col
...
takenSeats = set(getSeatId(seat) for seat in seats)
print(takenSeats)
for i in range(min(takenSeats), max(takenSeats)):
    if i not in takenSeats and (i-1) in takenSeats and (i+1) in takenSeats:
        print(i)